...
| 安全性风险 | 模型自主探索可能发现有害的行为模式   | 安全约束 RL（如 13.2 节讨论的） |
...
| Agentic RL（第 12 章）    | 自博弈可以训练工具使用策略——模型自己生成工具调用场景 |
| 测试时搜索（13.1 节）     | 自博弈学到的推理策略可以在推理时使用                 |
...
- **Agentic RL**：第 12 章讨论的方向，2025-2026 年最热门的研究方向之一
...
| 7    | DPO / KTO / SimPO      | 不需要 RL 的"对齐"方法             |
| 8    | GRPO、RLVR             | 可验证奖励 + 组内比较 = 推理能力   |
| 9    | SAC、TD3               | 连续动作空间的高效控制             |
| 10   | RLHF 流水线            | 工业级对齐的完整工程               |
| 11   | VLM RL                 | 视觉语言模型的强化学习             |
| 12   | Agentic RL             | 多轮工具交互的智能体训练           |
| 13   | 未来趋势               | 推理时搜索、具身智能、MARL、自博弈 |
