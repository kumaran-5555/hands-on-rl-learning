...
VisPlay 是一个很有创意的 VLM RL 框架，它的核心思想是让两个模型通过 RL **互相博弈、协同进化**——一个负责"出题"（Questioner），一个负责"答题"（Reasoner）。这和第 10 章讨论的 Self-Play 思想一脉相承，但专门为视觉场景设计。
...
```mermaid
flowchart LR
    subgraph Q["提问者 (Questioner)"]
        Q1["输入: 图片"] --> Q2["输出: 挑战性问题"]
    end

    subgraph R["推理者 (Reasoner)"]
        R1["输入: 图片 + 问题"] --> R2["输出: 回答"]
    end
...
```mermaid
flowchart TD
    IMG["图片输入"] --> VLM1["VLM 初步分析"]
    VLM1 --> TOOL1["工具 1: 图像标注\n(标出关键区域)"]
    TOOL1 --> VLM2["VLM 进一步推理"]
    VLM2 --> TOOL2["工具 2: 搜索验证\n(查找相关信息)"]
    TOOL2 --> VLM3["VLM 综合回答"]
    VLM3 --> OUTPUT["最终输出"]

    style VLM1 fill:#e3f2fd,stroke:#1565c0
    style VLM2 fill:#e3f2fd,stroke:#1565c0
    style VLM3 fill:#e3f2fd,stroke:#1565c0
    style TOOL1 fill:#fff3e0,stroke:#e65100
    style TOOL2 fill:#fff3e0,stroke:#e65100
```
...
VISTA-Gym 可以和 GRPO 自然结合。对同一个图片+问题，模型生成多组推理链（每组包含不同的工具调用序列），然后用规则奖励评估每组的质量，计算组内相对优势，更新策略。这和第 8 章的 GRPO 完全一致——只是生成的"回答"从纯文本变成了"工具调用序列 + 最终回答"。
...
VLM-RL 在机器人领域的应用前景最为广阔。机器人需要从摄像头输入中理解环境，然后做出操作决策。和第 9 章讨论的连续控制不同，VLM-RL 的核心是"用视觉理解来指导动作"——不是直接从像素到力矩，而是先理解"面前是什么物体"，再决定"该怎么操作"。

```mermaid
flowchart LR
    subgraph Traditional["传统机器人 RL"]
        PX["像素"] --> POL["策略网络"] --> ACT["力矩"]
    end

    subgraph VLMRL["VLM 机器人 RL"]
        PX2["像素"] --> VLMR["VLM 理解"]
        VLMR --> TXT["'前方有红色方块'"]
        TXT --> POL2["策略网络"] --> ACT2["力矩"]
    end

    style Traditional fill:#ffebee,stroke:#c62828
    style VLMRL fill:#e8f5e9,stroke:#2e7d32
```
...
**仿真精调**阶段，针对目标机器人的具体场景做精细化训练。这一步会引入域随机化（Domain Randomization，回顾第 9 章）——随机化光照、纹理、物体位置等参数，让策略在各种条件下都能工作。
...
## 11.3.5 从文本 RL 到多模态 RL：回顾与展望

回顾第 3 章到第 11 章的完整学习路径，我们看到 RL 从最简单的表格方法一路发展到了多模态的复杂场景：

| 章节              | 输入          | 动作空间      | 奖励来源  | 核心算法  |
| ----------------- | ------------- | ------------- | --------- | --------- |
| 第 4 章：DQN      | 状态向量/像素 | 离散          | 环境内置  | DQN       |
| 第 5 章：策略梯度 | 状态向量      | 离散/连续     | 环境内置  | REINFORCE |
| 第 6 章：PPO      | Token 序列    | 离散（token） | RM 打分   | PPO       |
| 第 8 章：GRPO     | Token 序列    | 离散（token） | 规则验证  | GRPO      |
| 第 9 章：连续控制 | 状态向量      | 连续（力矩）  | 环境内置  | SAC/TD3   |
| 第 11 章：VLM RL  | 图像 + Token  | 离散（token） | 规则+模型 | GRPO      |

贯穿始终的核心思想没有变——策略梯度定理（第 5 章）、Actor-Critic 架构（第 5 章）、PPO 的裁剪稳定性（第 6 章）、GRPO 的组内优势（第 8 章）。变化的是输入的模态、动作的空间和奖励的来源。这就是为什么我们在前几章花了大量篇幅打理论基础——这些基础在多模态场景中完全适用。
