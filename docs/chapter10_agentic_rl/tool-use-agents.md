...
下一节我们来看看工业界各家的 Agentic RL 实战经验——[工业界实战](./industrial-practice)。
...
RLER 的实现方式是：维护一个评分标准版本库，每隔 $N$ 步训练就根据模型的当前表现调整评分标准的严格程度。这和课程学习（10.2 节）有相似的思想——但 RLER 是"评分标准在演化"，而不是"任务难度在增加"。
