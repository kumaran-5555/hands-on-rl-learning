...
防范 Reward Hacking 的常见手段包括：使用多个互补的奖励信号、引入 adversarial testing、定期做 human evaluation、以及对奖励函数本身做红队测试（red-teaming）。

### Grounding：让智能体"脚踏实地"

**Grounding（接地/扎根）**是指智能体将其输出锚定到真实世界或外部知识的能力。一个没有 grounding 的 LLM 只能在参数记忆中"凭空想象"；而一个有 grounding 的 Agent 会通过工具交互来验证自己的输出——搜索确认事实、执行代码验证逻辑、查询数据库获取真实数据。

Grounding 是 Agentic RL 相对于纯文本 RL 的一大优势。通过 RL 训练，模型不仅学会了"调用工具"这个行为模式，更学会了**把工具作为认知延伸**——不确定的时候先查，推理有风险的时候先验证，信息不足的时候主动搜索。这种"脚踏实地"的行为习惯，是单纯 SFT 很难教出来的。

### Rejection Sampling：最简单的 RL

在正式进入复杂的策略优化之前，值得先了解一种最朴素但广泛使用的方法——**Rejection Sampling（拒绝采样）**，也叫 **Best-of-N**：
...
Rejection Sampling 的直觉非常好理解：如果你写 10 篇作文，挑出最好的那篇让老师批改，进步自然最快。但它的局限也很明显：**只能过滤，不能改进采样策略本身**。如果模型压根写不出正确的答案，采样 100 次 100 次都是错的，Rejection Sampling 就无能为力了。GRPO 本质上可以看作 Rejection Sampling 的策略梯度扩展——不只是挑出好答案，还要更新策略让好答案出现的概率更高。

### Self-play：让智能体和智能体对弈

**Self-play（自我博弈）**的经典案例是 AlphaGo——智能体通过与自己的历史版本对弈来持续提升。在 Agentic RL 中，self-play 有多种形态：

- **对抗式**：一个 Agent 出题，另一个 Agent 解题；出题的 Agent 学会出越来越难的题，解题的 Agent 被迫提升能力
- **协作式**：多个 Agent 协作完成复杂任务，通过团队奖励反向优化各自的策略
- **辩论式**：两个 Agent 对同一问题持不同立场辩论，裁判 Agent 判断谁更有说服力

Self-play 的核心吸引力在于：**不需要人类标注，训练信号自动生成**。但代价是可能陷入"策略漂移"——智能体群体可能收敛到一个自洽但不符合人类期望的局部最优。
...
- **SFT 教"格式"**：教会模型工具调用的语法（"调用搜索引擎的 JSON 长什么样"）、基本的交互协议
- **RL 教"策略"**：教会模型何时调用工具、如何组合多步行动、失败后如何恢复

DeepSeek-R1-Zero 的实验甚至表明，跳过 SFT 直接做 RL 也能涌现出推理能力——但前提是基座模型足够强大。实践中，SFT warmup + RL fine-tuning 的两阶段方案仍是主流。
...
| [动手：端到端 Agentic RL 训练——从目标到效果](./agentic-training-hands-on) | 定目标、搭环境、造数据、选策略、跑训练、看效果——完整管线实战 |
...
以上是 Agentic RL 的概念框架和关键术语。和第 1 章开篇一样，初次接触这些术语难免觉得密集——不必在此停留太久。后续各节会通过动手实验逐一展开，每遇到一个术语，你都会有具体的代码和训练曲线与之对应。
