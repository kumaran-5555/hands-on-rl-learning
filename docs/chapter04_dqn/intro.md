...
因此，本章的主线可以概括为一句话：**如何把表格 Q-Learning 扩展成一个能处理连续状态、像素输入和不稳定训练过程的深度强化学习算法。** 我们会先从 Q 表格的边界讲起，再拆开 DQN 的三个组件，随后统一用 LunarLander 观察训练曲线、消融实验和更复杂离散动作控制中的新问题，最后进入 Double DQN、Dueling DQN、Rainbow，以及从 LunarLander 迁移到视觉游戏的完整项目。
...
| [深度 Q 网络的三个组件](./dqn-components)                 | Q 网络、经验回放、目标网络分别解决什么问题？       |
| [LunarLander 训练分析](./training-analysis)               | 经验回放和目标网络各自起了什么作用？               |
| [LunarLander：更适合 DQN 的控制任务](./lunar-lander)      | DQN 在更复杂的离散动作控制任务上，会遇到什么问题？ |
