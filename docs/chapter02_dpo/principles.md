...
### 2.1.2 阶段二：监督微调（Supervised Fine-Tuning, SFT）
...
### 2.1.3 阶段三：强化学习与对齐（RL / Alignment）
...
- **数据示例（偏好三元组）**——摘自 [1-generate_data.py](../../code/chapter02_dpo/1-generate_data.py) 生成的真实训练数据：
  ```json
  {
    "prompt": "我今天心情很差，不想上班。",
...
trainer = DPOTrainer(
    ...
    tokenizer=tokenizer,
    beta=0.1,                 # β 系数：控制偏离 π_ref 的程度
)
```

代码中只传入了一个模型，没有奖励模型。`DPOTrainer` 会在内部自动创建一份 `model` 的冻结副本作为参考模型 $\pi_{ref}$，并在每一步训练中对比 $\pi_\theta$ 和 $\pi_{ref}$ 的输出概率。参数 `beta=0.1` 对应公式中的 $\beta$ 系数——后面推导损失函数时会反复提到它。
