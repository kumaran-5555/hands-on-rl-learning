# 第 9 章：连续控制——从离散动作到连续力矩

前面的八章里，我们的智能体一直在和"有限选项"打交道——CartPole 的"左推/右推"、Atari 的手柄按键、LLM 的 token 选择。这些场景有一个共同特征：动作是离散的、可数的，策略只需要回答"选哪一个"。
...
这一章，我们先在 PyBullet 仿真器里动手操控机器人，获得对连续动作空间的直觉。然后深入理论——理解高斯策略与确定性策略的区别，掌握 DDPG 和 TD3 如何将 DQN 的思想迁移到连续空间。最后，我们介绍目前连续控制领域最受欢迎的 SAC 算法，用对比实验帮你建立算法选择的工程判断。
